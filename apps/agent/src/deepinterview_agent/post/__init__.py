...
from .feedback_gen import generate_candidate_feedback
...
async def _run_score_locked(req: ScoreRequest, deps: Deps) -> ScoreCard:
    ...
    with start_trace("score", session_id=req.session_id):
        ...
        # Phase 7: AI-powered insights (optional enrichment, always guarded).
        insights = InterviewInsights()
        try:
            insights = await _guarded(
                generate_insights(ctx, scorecard, deps),
            ...
            log.debug("insights: generation skipped for %s", req.session_id)

        # Attach insights to the scorecard before persisting.
        scorecard.insights = insights

        # P0: Compute AI Match Score (0-100) from all available data.
        try:
            match = compute_match_score(scorecard, insights)
            scorecard.match_score = match
        except Exception:
            log.debug("match_score: computation failed for %s", req.session_id)
            scorecard.match_score = MatchScoreBreakdown()

        # P0: Auto-generate candidate feedback (best-effort, non-fatal).
        try:
            feedback = await _guarded(
                generate_candidate_feedback(ctx, scorecard, deps),
                label="generate_feedback",
                timeout=timeout,
            )
            scorecard.candidate_feedback = feedback or CandidateFeedback()
        except Exception:
            log.debug("feedback: generation skipped for %s", req.session_id)
            scorecard.candidate_feedback = CandidateFeedback()
